(function($) {
    ...
    $.mcDropDownMenu = function(el, list, options) {
        ...
        $list, $divInput, settings, typedText = "",
        matchesCache, oldCache, $keylist, $keylistiframe, bInput, bDisabled = false;
        $self = $(el);
        ...
        $divInput = $('<div class="mcdropdown"><a href="#" tabindex="-1"></a><input type="hidden" name="' + (el.name || el.id) + '" id="' + (el.id || el.name) + '" /></div>').appendTo($('<div style="position: relative;"></div>')).parent();
        ...
        $self.width(di.width).filter(":input").attr("autocomplete", "off").bind("keypress", checkKeypress).bind("mousedown",
        function(e) {
        ...
        if ($.browser.msie || $.browser.safari) $self.bind("keydown",
        function(e) {
            if (",8,9,37,38,39,40,".indexOf("," + e.keyCode + ",") > -1) return checkKeypress(e)
        });
        $("a", $divInput).bind("click",
        function(e) {
        ...
        this.openMenu = function(e) {
            ...
            function open() {
                ...
                $list[settings.openFx](settings.openSpeed,
                function() {
                    scrollToView($list)
                });
        ...
        function parseTree($selector) {
            var s = [],
            level = (arguments.length > 1) ? ++arguments[1] : 1;
        ...
        function getPadding(el, name) {
            ...
            return (parseInt("0" + $(el).css("border" + torl + "Width"), 10) + parseInt("0" + $(el).css("border" + borr + "Width"), 10) + parseInt("0" + $(el).css("padding" + torl), 10) + parseInt("0" + $(el).css("padding" + borr), 10) + parseInt("0" + $(el).css("margin" + torl), 10) + parseInt("0" + $(el).css("margin" + borr), 10))
        };
        ...
        function columnizeList($el) {
            ...
            $children.each(function(i) {
                ...
                $li.css({
                    ...
                    "marginTop": marginTop,
                    "width": widths.item
                })[((nextItemColumn > column) || (currentItem == items)) ? "addClass": "removeClass"]("mc_endcol")[(marginTop != 0) ? "addClass": "removeClass"]("mc_firstrow");
        ...
        function showMatches($li, hideResults) {
            var bCached = ($li === oldCache),
            $items = bCached ? $keylist.find("> li").removeClass("mc_hover mc_hover_parent mc_firstrow") : $li.clone().removeAttr("style").removeClass("mc_hover mc_hover_parent mc_firstrow mc_endcol").filter(":last").addClass("mc_endcol").end();
        ...
        function checkKeypress(e) {
            ...
            if (matchesCache.length > 0) {
                ...
                matchesCache = findMatches($lis, compare);
                if (matchesCache.length > 0) showMatches(matchesCache);
                else hideMatches()
            }
            e.preventDefault();
            return false
