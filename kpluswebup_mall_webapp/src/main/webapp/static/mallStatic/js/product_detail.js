...
function addCat(itemID,productID,stock,supplierID){
	...
	if (customerID == ""){
		var preUrl = '/mall/product/productDetail.htm?productID='+productID+'---itemID='+itemID;
		window.location= domain+'/mall/product/toLogin.htm?preUrl='+preUrl;
		//window.location= domain+'/mall/product/productDetail.htm?productID='+productID+'&itemID='+itemID;
		//mall/product/productDetail.htm?productID=01D5B540-FB86-41A8-92AA-FF5E79E72343&itemID=0003517647845
		//window.location.href="/mall/member/toLogin.htm";
		return;
	}
	jQuery.ajax({
		...
		success : function(data) {
			if (data.code == 0) {
				$(".waite").show();
				window.location= domain+'/shoppingCart1.htm';
			} else {
				alert("添加失败！");
			}
		},
...
function gobuy() {
	...
	window.location.href = "../../goBuy.htm?cartID=" + itemID + "&number="
			+ itemCount + "&orderType=" + orderType+"&productID="+productID+"&supplierID="+supplierID;
}
