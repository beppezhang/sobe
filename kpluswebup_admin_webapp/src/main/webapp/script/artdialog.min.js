...
var $ = window.art = function(selector, context){
	...
	/*	(xyz)        匹配xyz,并捕获文本到自动命名的组里
		[xyz]        字符集。匹配包含的任一字符。例如，"[abc]"匹配"plain"中的"a"。  
		[^<]         匹配除<以外的字符
		*            零次或多次匹配前面的字符，等效于{0,}
		(<[\w\W]+>)  匹配<任意字符非字符集>
		[^>]         匹配除>以外的字符
		#([\w\-]+)   匹配#和一个或多个A-Z、a-z、0-9_-字符组成的字符集
		/^(?:[^<]*(<[\w\W]+>)[^>]*$|#([\w\-]+)$)/  匹配不包含<和>的由A-Z、a-z、0-9_-#组成的字符集
	*/
    quickExpr = /^(?:[^<]*(<[\w\W]+>)[^>]*$|#([\w\-]+)$)/,
	/*
		\n 标识一个八进制转义码或反向引用
...
+										'<div class="d-title" id="dialogTitle"></div>'
+										'<a class="d-close" id="dialogClose" href="javascript:void(0);">'
+											'\xd7'
+										'</a>'
+									'</div>'
+								'</td>'
...
+							'<tr>'
+								'<td class="d-main">'
+									'<div class="d-content" id="dialogContBox"></div>'
+								'</td>'
+							'</tr>'
+							'<tr>'
+								'<td class="d-footer">'
