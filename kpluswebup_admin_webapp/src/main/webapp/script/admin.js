	...
	$("#exportMember").click(function() {
		...
		if (confirm("确定要导出数据吗？")) {
			window.location.href = "exportMember.do?searchUserName=" + searchUserName+"&searchName="+searchName+"&searchMobile="
														+searchMobile+"&searchEmail="+searchEmail+"&gradeID="+gradeID+"&searchStatus="
														+searchStatus+"&provinceID="+provinceID+"&cityID="+cityID+"&districtID="
														+districtID;
		} else {
			return false;
		}
...
$(function() {
	$("#upProduct").click(function() {
		...
		if (vals != "") {
			...
			updateProductStatus(vals, 0);		//isDelete=0(借用原来的结构，这里改参相对简便)
		} else {
			alert("请选择要上架的产品");
		}
	});
