(function($) {
    function RollBar(c, s) {
        ...
        this.container.css({
            'overflow': 'hidden',
            'position': 'relative'
        }).contents().wrapAll('<div class="rollbar-content"></div>');
    ...
    RollBar.prototype.scroll = function(v, h) {
        var a = 0;
        var b = 0;
        if (v < 0) {
            v = 0
    ...
    RollBar.prototype.init = function() {
        ...
        this.bindEvent($(window), 'resize', function() {
            this.pathSize();
            this.checkScroll();
            this.scroll(Math.round(this.vslider.position().top * this.vpath.height() / g), Math.round(this.hslider.position().left * this.hpath.width() / htrack));
            g = this.vpath.height();
            htrack = this.hpath.width()
        });
        this.bindEvent(this.container, 'mousewheel', function(e, a, b, c) {
            this.scroll(this.vslider.position().top - this.settings.wheelSpeed * c, this.hslider.position().left + this.settings.wheelSpeed * b);
            this.sliders.stop().fadeTo(this.settings.sliderOpacityTime, 1);
        ...
        this.bindEvent($(window), 'load', function() {
            setInterval(this.fixFn(this.checkScroll), 10)
        })
    };
